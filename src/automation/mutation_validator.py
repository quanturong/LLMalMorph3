...
class CodeFeatures:
    ...
    # Invalid char literal assignments generated by string obfuscation.
    # e.g. `_s0[3]='\';` is syntactically invalid; it should be `'\\'`.
    invalid_char_literals: List[Tuple[str, str]] = field(default_factory=list)

    # Variables declared in a nested block and then used after that block.
    # e.g. `if (...) { T x = ...; } use(x);`
    scope_escape_uses: List[Tuple[str, int, int, str]] = field(default_factory=list)

    # C/C++ goto labels present in the function body.
...
class _FeatureExtractor:
    ...
    def extract(self, code: str, language: str = "c") -> Optional[CodeFeatures]:
        ...
        self._extract_invalid_char_literals_regex(code, feats)
        self._extract_scope_escape_uses_regex(code, feats)
        self._extract_labels_regex(code, feats)
    ...
    def _extract_invalid_char_literals_regex(self, code: str, feats: CodeFeatures):
        """Detect malformed char literal assignments in generated char arrays."""
        # Unterminated backslash char literal: _s0[3]='\';
        for m in re.finditer(r"\b(\w+\s*\[\s*\d+\s*\]\s*=\s*'\\'\s*;)", code):
            feats.invalid_char_literals.append(("unterminated_backslash_char", m.group(1)[:120]))

        # Multi-character constants created from escaped string fragments.
        # Valid examples include '\\n', '\\r', '\\\\', '\\'', '\\x41', '\\123'.
        valid_escape = re.compile(r"""^\\(?:[abfnrtv\\'"?]|x[0-9A-Fa-f]+|[0-7]{1,3})$""")
        for m in re.finditer(r"\b(\w+)\s*\[\s*\d+\s*\]\s*=\s*'([^'\n\r]*)'\s*;", code):
            literal = m.group(2)
            if len(literal) <= 1:
                continue
            if literal.startswith("\\") and valid_escape.match(literal):
                continue
            feats.invalid_char_literals.append(("invalid_multi_char_literal", m.group(0)[:120]))

    def _extract_scope_escape_uses_regex(self, code: str, feats: CodeFeatures):
        """Detect local declarations whose scope is narrower than their later use."""
        decl_re = re.compile(
        ...
        for line_no, line in enumerate(code.splitlines(), 1):
            ...
            if match:
                ...
                if first_word not in keyword_types and not var_name.startswith("_junk"):
                    # Only nested declarations can escape their scope.
                    if depth_at_line >= 2:
                        declarations.setdefault(var_name, (depth_at_line, line_no))

            for var_name, (decl_depth, decl_line) in list(declarations.items()):
                if line_no <= decl_line:
                    continue
                if depth_at_line < decl_depth and re.search(r'\b' + re.escape(var_name) + r'\b', code_line):
                    feats.scope_escape_uses.append((var_name, decl_depth, depth_at_line, code_line.strip()[:120]))
                    declarations.pop(var_name, None)

            # Approximate brace depth after checking the current line. This is
            # enough for the generated straight-line C/C++ we validate here.
            brace_depth += code_line.count("{") - code_line.count("}")
            brace_depth = max(0, brace_depth)

    def _extract_labels_regex(self, code: str, feats: CodeFeatures):
        """Extract non-switch labels used by goto-style control flow."""
        label_re = re.compile(
            r'^\s*(?!case\b|default\b|public\b|private\b|protected\b)'
            r'([A-Za-z_]\w*)\s*:\s*(?:$|//|/\*)',
            re.MULTILINE,
        )
        feats.labels.update(label_re.findall(code))
...
def _check_invalid_char_literals(
    orig: CodeFeatures, mut: CodeFeatures, strategy: str,
) -> CheckResult:
    """Reject malformed char literals emitted by string obfuscation."""
    orig_bad = {snippet for _kind, snippet in orig.invalid_char_literals}
    new_bad = [(kind, snippet) for kind, snippet in mut.invalid_char_literals if snippet not in orig_bad]
    if new_bad:
        kind, snippet = new_bad[0]
        return False, (
            f"Invalid character literal generated by mutation ({kind}): `{snippet}`. "
            f"Use valid C/C++ char escapes: newline is `'\\n'`, carriage return is `'\\r'`, "
            f"and a literal backslash is `'\\\\'`. Do not emit a bare backslash char literal."
        )
    return True, ""


def _check_scope_escape_uses(
    orig: CodeFeatures, mut: CodeFeatures, strategy: str,
) -> CheckResult:
    """Reject declarations moved into a nested scope but used later outside it."""
    orig_escapes = {item[0] for item in orig.scope_escape_uses}
    new_escapes = [item for item in mut.scope_escape_uses if item[0] not in orig_escapes]
    if new_escapes:
        var_name, decl_depth, use_depth, snippet = new_escapes[0]
        return False, (
            f"Variable scope regression: `{var_name}` is declared inside a nested block "
            f"(depth {decl_depth}) but later used after that block (depth {use_depth}) in "
            f"`{snippet}`. Declare it in the same or wider scope as the original variable, "
            f"and only assign it conditionally inside the block."
        )
    return True, ""


def _check_label_preservation(
    orig: CodeFeatures, mut: CodeFeatures, strategy: str,
) -> CheckResult:
    """Reject mutations that remove labels from label/goto functions."""
    missing = orig.labels - mut.labels
    if missing:
        return False, (
            f"Label preservation failure: mutated code removed label(s) "
            f"{', '.join(sorted(missing))}. Functions with labels/goto-like "
            f"control flow must preserve every original label inside the same "
            f"function body and keep goto targets reachable."
        )
    return True, ""
...
_CHECKS: List[Tuple[str, CheckFn]] = [
    ...
    ("invalid_char_literals",    _check_invalid_char_literals),
    ("scope_escape_uses",        _check_scope_escape_uses),
    ("label_preservation",       _check_label_preservation),
...
_CHECK_TO_POLICY_GATES = {
    ...
    "invalid_char_literals": {"string_buffer_bounds"},
    "scope_escape_uses": {"string_buffer_bounds", "helper_scope"},
